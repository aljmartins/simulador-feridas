...
with tabs[2]:
    ...
    if modo == "Formulário":
        ...
        if st.button("Avaliar caso do estudante", key=f"{K_ESTUDANTE}_avaliar_form"):
            ...
            _set_export_payload(origem="Estudante: inserir caso", caso=st.session_state.get(f"{K_ESTUDANTE}_dados"), plano_ideal=st.session_state.get(f"{K_ESTUDANTE}_ideal",""))
