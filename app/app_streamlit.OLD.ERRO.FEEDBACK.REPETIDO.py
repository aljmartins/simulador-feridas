...
with tabs[2]:
    ...
    if not st.session_state.get(f"{K_ESTUDANTE}_dados"):
        ...
        with coly:
            if st.button("Gerar feedback (Gemini)", key=f"{K_ESTUDANTE}_gerar_feedback"):
                if not estudante_plano.strip():
                    ...
                    try:
                        ...
                        # Se o Gemini pedir mais dados, fica bem claro
